...
- [Anonimização de dados]{.alert}: para remoção de ruídos.  
- [Geração de hierarquia de conceitos]{.alert}: dados primitivos são substituídos por conceitos de ordem superior utilizando-se uma hierarquia de conceitos. 
  - **Exemplo:** atributo Rua $\rightarrow$ atributo Cidade ou País.  
- [Normalização]{.alert}: ajuste de escala.
- [Discretização]{.alert}: reduz a quantidade de valores de um atributo contínuo pela divisão da amplitude em intervalos. Os rótulos dos intervalos substituem os valores originais.  
